...
def upgrade() -> None:
    """Expand legacy `namespace` into `namespaces` without removing it."""
    bind = op.get_bind()
    rows = bind.execute(
        sa.text(
    ...
    for row in rows:
        ...
        if namespaces or not legacy_namespace:
            continue
...
def downgrade() -> None:
    """Remove the new key while preserving/restoring a legacy namespace."""
    bind = op.get_bind()
    rows = bind.execute(
        sa.text(
            "SELECT id, config FROM application_tools "
