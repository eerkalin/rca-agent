...
from app.integrations.llm.factory import SUPPORTED_LLM_PROVIDER_TYPES
...
def _validate_llm_connection(db, connection_id: int | None) -> None:
    ...
    if connection.provider_type not in SUPPORTED_LLM_PROVIDER_TYPES:
        raise HTTPException(
            status_code=422,
            detail=f"Connection {connection_id} is not a supported LLM provider",
        )
