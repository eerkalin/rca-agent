...
from app.observability.logging import elapsed_ms, log_event, redact
...
async def test_connection(connection_id: int):
    ...
    with SessionLocal() as db:
        ...
        try:
            ...
            log_event(logger, logging.ERROR, "connection.test.failure", "Connection test failed", connection_id=connection_id, connection_name=connection.name, provider_type=connection.provider_type, elapsed_ms=elapsed_ms(started), error_type=type(exc).__name__, error=safe_error)
            logger.exception("Connection test failed for connection_id=%s provider=%s", connection_id, connection.provider_type)
            raise HTTPException(status_code=400, detail={"message": "Connection test failed", "provider": connection.provider_type, "error_type": type(exc).__name__, "error": safe_error}) from exc
