...
class PrometheusProvider:
    ...
    def test_connection(self) -> dict:
        ...
        return {"connected": True, "provider": "prometheus", "result_count": len(payload.get("data", {}).get("result", []))}
    ...
    def range_query(self, promql: str, start: datetime | None = None, end: datetime | None = None, step: str | int | None = None) -> list[dict]:
        end = end or datetime.now(timezone.utc)
        start = start or (end - timedelta(minutes=15))
        params = {"query": promql, "start": start.timestamp(), "end": end.timestamp(), "step": step or self.config.get("default_step", "30s")}
    ...
    def collect_configured_queries(self, queries: list[dict], variables: dict[str, Any], default_window_minutes: int = 15) -> dict:
        items = []
