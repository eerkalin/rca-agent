...
def test_expected_head_revision_is_current_release_head():
    assert schema_compat.expected_head_revisions() == {"e3f4a5b6c7d8"}
...
def test_schema_status_reports_current(monkeypatch):
    ...
    monkeypatch.setattr(schema_compat, "expected_head_revisions", lambda: {"e3f4a5b6c7d8"})
    status = schema_compat.schema_status(object())
    assert status["up_to_date"] is True
    assert status["current_revisions"] == ["e3f4a5b6c7d8"]
